...
// One reusable OpenCV image per SDK device. The receive callback overwrites it.
static cv::Mat lidar_img_data[iTFS::max_device];

// The queue carries device indexes only; pixels stay in lidar_img_data.
static std::condition_variable lidar_cv;
static std::mutex lidar_cv_mutex;
static std::queue<int> lidar_q;

// Image callback: copy SDK-owned pixels before returning, then wake the UI loop.
