/**
 * @file pcl_example.cpp
 * @brief iTFS PCL example
...
// Per-pixel direction vectors loaded from the calibration table beside the executable.
...
int readIntrinsic(std::string file) {
    ...
    if (fp == NULL) {
        printf("[ERROR] iTFS::LiDAR intrinsic file was not found: %s\n", file.c_str());
        return (-1);
    }
...
// Resolve resources relative to the executable rather than the working directory.
static std::string getBinDir(char *argv0) {
...
// Spacing between multiple lidars
constexpr float spacing = 2.5f;

// Rotation for lidars [radian] (yaw->pitch->roll)
constexpr float yaw[iTFS::max_device] = {
    0.0f,
    0.0f,
};
constexpr float pitch[iTFS::max_device] = {
    0.0f,
    0.0f,
};
constexpr float roll[iTFS::max_device] = {
    0.0f,
    0.0f,
};
...
// Image callback: copy SDK-owned pixels before returning, then wake the viewer loop.
...
int main(int argc, char *argv[]) {
    ...
    std::string calibFile = getBinDir(argv[0]) + "/iTFS-110.dat";
    // std::string calibFile = getBinDir(argv[0]) + "/iTFS-80.dat";
    if (readIntrinsic(calibFile) < 0) {
        delete ilidar;
        return (-1);
    }
    ...
    while (true) {
        ...
        R.rotate(Eigen::AngleAxisf(yaw[recv_device_idx], Eigen::Vector3f::UnitZ()) *
                 Eigen::AngleAxisf(pitch[recv_device_idx], Eigen::Vector3f::UnitY()) *
                 Eigen::AngleAxisf(roll[recv_device_idx], Eigen::Vector3f::UnitX()));
        pcl::transformPointCloud(*scan[recv_device_idx], *scan[recv_device_idx], R);

        // Apply spacing for multiple lidars
        Eigen::Affine3f transform = Eigen::Affine3f::Identity();
        transform.translation() << 0.0f, (recv_device_idx * spacing), 0.0f;
