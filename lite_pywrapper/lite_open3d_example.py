...
class Open3DPointCloudViewer:
    ...
    def __init__(self, o3d):
        ...
        if not self.vis.create_window("iTFS-LITE Open3D Point Cloud", width=960, height=720):
            raise RuntimeError(
...
def lidar_data_handler(viewer, img):
    """Convert one LITE frame in native code and update its device cloud."""
    # Corresponds to C++ lidar_data_handler().
    # `img` is the per-device LiteImgCpy overwritten by the native data event.
    cloud = img.point_cloud()
    if cloud is None or len(cloud) == 0:
        print("no point cloud from frame", frame_summary(img), "map", img.reconstruction_map_valid)
        return True
    cloud = apply_device_transform(cloud, img.idx)
    return viewer.update(img.idx, cloud)
    ...
    with open_lite(ilidar_lite, args, queue_size=4) as lite:
        ...
        try:
            while True:
                ...
                if not lidar_data_handler(viewer, img):
                    break
